...
function isIEFunction() { //LOGIFIX 22307 Analysis Grid:`Member not found' js error is shown on resizing Heatmap  in IE7
...
var UNDEFINED,
     ...
     */
    function hash() { //LOGIFIX First results 10/4/2013 by Dmitry
        var i = 0,
            args = arguments,
            length = args.length,
            obj = {};
...
function getTextMargins(element) {
    if (!element)
        return 0;

    var margin = 0;

    each(element.children, function (node) {
        var childMargin = 0;
        var styles = window.getComputedStyle(node);

        if (styles.marginLeft)
            childMargin += parseFloat(styles.marginLeft);
        if (styles.marginRight)
            childMargin += parseFloat(styles.marginRight);
        if (styles.paddingLeft)
            childMargin += parseFloat(styles.paddingLeft);
        if (styles.paddingRight)
            childMargin += parseFloat(styles.paddingRight);
        if (styles.borderLeftWidth)
            childMargin += parseFloat(styles.borderLeftWidth);
        if (styles.borderRightWidth)
            childMargin += parseFloat(styles.borderRightWidth);

        childMargin += getTextMargins(node);
...
function normalizeTickInterval(interval, multiples, magnitude, allowDecimals) {
     ...
     */
function ChartCounters() { //LOGIFIX First results 10/4/2013 by Dmitry
        this.color = 0;
        this.symbol = 0;
    }
...
defaultOptions = {
	...
	lang: {
		...
		resetZoom: 'Reset zoom', //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework
	...
	chart: {
        ...
        position: {
                align: 'right',
                x: 0,
                verticalAlign: 'top', //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework
                y: 1
        }	            
}
	},
	title: {
		...
		// y: null,
		style: {
		    color: '#274b6d',//#3E576F', //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework and reverting of formatters changes
            fontSize: '16px'
		}
	...
	subtitle: {
		...
		// y: null,
		style: {
		    color: '#4d759e' //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework and reverting of formatters changes
		}
	},
...
SVGElement.prototype = {
	...
	attr: function (hash, val) {
            //LOGIFIX 23213 - crashes sometime after ajax refresh
	...
	*/
	 ...
	 *
	 */
	align: function (alignOptions, alignByTranslate, box, point) { //LOGIFIX Issue 21976 and Revert "Issue 21976 Data labels (Alignment horizontal and vertical) are not working for chart series"
		var align,
			vAlign,
			x,
			y,
	    ...
	    if (point && point.series) {
	        if (point.series.type === 'bubble') {
	            ...
	            if (point.series.type === 'column' || point.series.type === 'columnrange') {
	                if (align === 'left') {
	                    x -= (box.width) / 2;
	                }
	                if (align === 'right') {
	                ...
	                if (align === 'center') {
	                    x += (box.width - (alignOptions.width || 0)) / 2;
	                }
	            } else {
	                if (align === 'left' || align === 'center') {
	                    x += (box.width - (alignOptions.width || 0)) /
	                    { left: 1, center: 2 }[align];
	                }
	            }
	        }
	    } else {
		...
		attribs[alignByTranslate ? 'translateX' : 'x'] = mathTrunc(x);


	    // Vertical align //LOGIFIX
	    if (point && point.series) {
	        if (point.series.type === 'bubble' || point.series.type === 'column') {
	    ...
	    attribs[alignByTranslate ? 'translateY' : 'y'] = mathTrunc(y);
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, style, forExport) {
		...
		renderer.url = (isFirefox || isWebKit) && doc.getElementsByTagName('base').length ?
			...
			'';
	    //LOGIFIX Code here was removed. Issue 23756 Section 508 Accessibility for Canvas Charts
	...
	buildText: function (wrapper) {
	    var textNode = wrapper.element,
			...
			hasMarkup = textStr.indexOf('<') !== -1 || textStr.indexOf('&lt;') != -1,
			lines,
	    ...
	    if (!hasMarkup && !textStroke && textStr.indexOf(' ') === -1) {
	        ...
	        if (hasMarkup) {
	            lines = textStr.split(/<br.*?>/gi);
	            for (var i = 0; i < lines.length; i++) {
	                lines[i] = lines[i].replace(/<(b|strong)/gi, '<span style="font-weight:bold"')
	                	               .replace(/<(i|em)/gi, '<span style="font-style:italic"')
	                	               .replace(/<a/gi, '<span')
	                	               .replace(/<\/(b|strong|i|em|a)>/gi, '</span>');
	            }
	        } else {
	            lines = [textStr];
	        }
...
Tick.prototype = {
	...
	addLabel: function () {
		...
		// update
		} else if (label) {//LOGIFIX fore null attr returning
		    var t = label.attr({
		        text: str
		    });
		    if (t) {
	...
	handleOverflow: function (index, xy) {
		...
		if (justifyLabel) {
		    ...
		    if (isLast) {
		        var textDimension = LogiXML.layout.getTextDimensions(this.label.textStr, this.label.styles);
		        var chart = document.getElementById(axis.chart.options.id);
		        if (chart) {
		            var chartDimensions = chart.getBoundingClientRect();
		            var labelStartPosition = xy.x - leftSide;
		            var labelEndPosition = labelStartPosition + textDimension.width;
		            if (labelEndPosition >= chartDimensions.width) {
		                justifyToPlot = true;
		            }
		        }
		    }
...
Axis.prototype = {
	...
	getPlotLinePath: function (value, lineWidth, old, force, translatedValue) {
		...
		if (isNaN(translatedValue)) { // no min or max
		    ...
		    // LOGIFIX
			if (y1 < axisTop) { // Show marker if its y-axis coordinate less than 0
				skip = true;
			}
		}
		return skip && !force ?
	...
	setTickPositions: function (secondPass) {
		var axis = this,
            ...
            minTickInterval, //LOGIFIX 24604 AC: Line charts X-axis has extra points when data grouped by date by Year
		...
		if (!categories && !axis.axisPointRange && !axis.usePercentage && !isLinked && defined(axis.min) && defined(axis.max)) {
			...
			if (length) {
			    if (!defined(options.min) && !defined(axis.userMin) && minPadding &&
                    (axis.dataMin < 0 || !axis.ignoreMinPadding) || axis.chart.options.chart.type == "gauge") {
			        //LOGIFIX - if data values >= 0 and axis.min is not set, then force it to be zero, otherwize it will start from extra tick with negative value
			        if (axis.dataMin >= 0 && !axis.isDatetimeAxis && !axis.isXAxis && !axis.isLog) {
			            if (axis.min - length * minPadding < 0) {
			                options.min = 0;
			            } else {
			    ...
			    else if (axis.dataMin < 0 && !axis.chart.angular) {
		...
		if (axis.pointRange && !axis.tickInterval) { //LOGIFIX 24429: ChartCanvas X-Axis scales not as expected
		    axis.tickInterval = axis.pointRange;
		}

	    // Before normalizing the tick interval, handle minimum tick interval. This applies only if tickInterval is not defined.
		minTickInterval = pick(options.minTickInterval, axis.isDatetimeAxis && axis.closestPointRange); //LOGIFIX 24604 AC: Line charts X-axis has extra points when data grouped by date by Year
		...
		if (!isLinked && !axis.options.holdLastTick) { //LOGIFIX: Fix for gauges
	...
	setAxisSize: function () {
		...
		this.top = top;
		this.width = width;
		this.height = height - ((chart.legend && chart.legend.baseline) ? chart.legend.baseline : 0); //LOGIFIX 22972 //LOGIFIX font scaling
	...
	render: function () {
		...
		do {//LOGIFIX - labels and ticks in SVG gauges
		...
		if (hasData || isLinked) {
			...
			if (tickPositions.length) { // #1300
				...
				if (justifyLabels) {
				    if (!axis.options.holdLastTick) {//LOGIFIX - labels and ticks in SVG gauges
				        sortedPositions = sortedPositions.slice(1).concat([sortedPositions[0]]);
				    } else {
				        if (/*sortedPositions[sortedPositions.length - 1] > axis.options.max*//*ticks.length>0*/true) {
				            if (tickPositions.length > 0) {
				                ticks[sortedPositions[sortedPositions.length - 1]].pos = axis.options.max;
				                var newLastTick = ticks[sortedPositions[sortedPositions.length - 1]];
				                ticks[sortedPositions[sortedPositions.length - 1]].destroy();
				                delete ticks[sortedPositions[sortedPositions.length - 1]];
				                if (ticks[axis.options.max]) {
				                    ticks[axis.options.max].destroy();
				                }
				                ticks[axis.options.max] = null;
				            }
				            sortedPositions[sortedPositions.length - 1] = axis.options.max;
				            tickPositions[tickPositions.length - 1] = axis.options.max;
				            axis.tickPositions = tickPositions;
				            axis.max = axis.options.max;
				            axis.ticks = ticks;
				        }
				        if (sortedPositions[0] < axis.options.min/* && ticks.length>0*/) {
				            if (tickPositions.length > 0) {
				                ticks[sortedPositions[0]].pos = axis.options.min;
				                var newLastTick = ticks[sortedPositions[0]];
				                ticks[sortedPositions[0]].destroy();
				                delete ticks[sortedPositions[0]];
				                if (ticks[axis.options.min]) {
				                    ticks[axis.options.min].destroy();
				                }
				                ticks[axis.options.min] = null;
				            }
				            sortedPositions[0] = axis.options.min;
				            tickPositions[0] = axis.options.min;
				            axis.tickPositions = tickPositions;
				            axis.min = axis.options.min;
				            axis.ticks = ticks;
				        }
				        var referentTickCount = parseInt((Math.abs(axis.options.max - axis.options.min)) / axis.options.tickInterval);
				        if ((axis.tickPositions.length - 1) > referentTickCount && sortedPositions.length > 2) {
				            var index = sortedPositions.length - 2;
				            ticks[sortedPositions[index]].destroy();
				            delete ticks[sortedPositions[index]];
				            axis.ticks = ticks;
				            sortedPositions.removeAt(index);
				            tickPositions.removeAt(tickPositions.length - 2);
				            axis.tickPositions = tickPositions;
				        }
				    }
				}
			        ...
			        each(sortedPositions, function (pos, i) {
			            ...
			            if (axis.options.holdLastTick) {//LOGIFIX - labels and ticks in SVG gauges
			                if (pos === sortedPositions.length - 1) {
			                    i = sortedPositions.length - 2;
			                }
			            }
		...
		} while (axis.decreaseTickCountIfOverlap && !axis.decreaseTickCountIfOverlap(axis, ticks, sortedPositions, tickPositions)) //LOGIFIX SVG gauges tick fixes and Arc gauge font size fixes.
...
Tooltip.prototype = {
	...
	getAnchor: function (points, mouseEvent) {
		...
		if (!ret) {
			each(points, function (point) {
			    if (point.series && point.series.yAxis) { // LOGIFIX 23444: ChartCanvas: JS error after fast random rotating
	...
	refresh: function (point, mouseEvent) {
		...
		if (text === false) {
			...
			if (tooltip.isHidden) {
				...
				if (label.div) { //LOGIFIX Issue 24846 ChartCanvas: Quicktip not shown when chart initially hidden
				    label.div.style.zIndex = label.div.style.zIndex || 1; 
				}
			}
	...
	updatePosition: function (point, mouseEvent) {
	    ...
	    if (this.label.div) {
	        var childSpan = this.label.div.children[0];
		    var width = Number.MIN_VALUE;

		    var rows = [].slice.call(this.label.div.getElementsByTagName("tr"));

		    var header = this.label.div.getElementsByClassName("header");
		    if (header && header.length > 0)
		        rows = rows.concat([].slice.call(header));

		    var body = this.label.div.getElementsByClassName("body")[0];
		    if (body) {
		        var pTag = body.getElementsByTagName("p");
		        if (pTag && pTag.length > 0)
		            rows = rows.concat([].slice.call(pTag));

		        var bTag = body.getElementsByTagName("b");
		        if (bTag && bTag.length > 0)
		            rows = rows.concat([].slice.call(bTag));

		        var spanTag = body.getElementsByTagName("span");
		        if (spanTag && spanTag.length > 0)
		            rows = rows.concat([].slice.call(spanTag));

		        rows = rows.concat(body);
		    }

		    if (childSpan) {
		        childSpan.style.width = "";
		        childSpan.style.height = "";
		    }

		    each(rows, function (row) {
		        if (row) {
		            var styles = window.getComputedStyle(row);
		            var dimensions = getTextDimensions(row.innerHTML.replace(/(<\/td>.*<td>)/gmi, "\t"), styles);
		            var rect = row.getBoundingClientRect();

		            var actualWidth = Math.max(dimensions.width, rect.width);
		    ...
		    if (width >= this.chart.plotWidth)
		        width = Math.ceil(this.chart.plotWidth - 10);
		    else
		        width = Math.ceil(width+ getTextMargins(childSpan));
		    
		    this.label.width = width;
		    this.label.div.style.width = width + "px";            	    

		    if (childSpan) {
		        var rect = childSpan.getBoundingClientRect();
		        var height = mathRound(rect.height);

		        this.label.height = height;
		        this.label.div.style.height = height + "px";

		        childSpan.style.width = width + "px";
		        childSpan.style.height = height + "px";
		    }

		    this.label.element.style.display = "none";
		}

		var pos = (this.options.positioner || this.getPosition).call(this, label.width, label.height, point);

		if (point.bar && point.bar.shapeType == 'rect' && mouseEvent &&
            this.chart.tooltip && !this.chart.tooltip.followPointer) { // #25457
...
Pointer.prototype = {
		...
		if (this.hasDragged > 10) {
			...
			if (chart.hasCartesianSeries && (this.zoomX || this.zoomY) && clickedInside && !panKey) {
				if (!this.selectionMarker) {
					...
					.add();
                        //LOGIFIX for chart selection 22191 ChartCanvas: Reverting of an upgrade to v4 framework
                        // Mask selection 22191 ChartCanvas: Reverting of an upgrade to v4 framework and reverting of formatters changes
					fireEvent(this.chart, 'selectionStarted', {selectionMarker: this.selectionMarker});
				}
			}
	...
	drop: function (e) {
		...
		if (this.selectionMarker) {
		    var selectionBox = this.selectionMarker, //LOGIFIX 23620: ChartCanvas: InputSelection.Range stopped to work after upgrading Highcharts ver
                ...
                selectionData = {
                    ...
                    //LOGIFIX
                    // Mask selection 22191 ChartCanvas: Reverting of an upgrade to v4 framework
                    selectionBox: { //22191 ChartCanvas: Reverting of an upgrade to v4 framework
                        x: selectionLeft,
                        y: selectionTop,
...
extend(Highcharts.Pointer.prototype, {
		...
		if (e.touches.length === 1) {
			...
			if (chart.isInsidePlot(e.chartX - chart.plotLeft, e.chartY - chart.plotTop)) {
				...
				// Run mouse events and display tooltip etc
                    //LOGIFIX - for mask selection ChartCanvas: chart selection #7
                    if (this.options.chart.customSelection) {
                        ...
                        // the series click event
                        fireEvent(chart.hoverPoint.series, 'click', extend(e, {
                            point: chart.hoverPoint
                        }));
                        // the point click event
                        if (chart.hoverPoint) { // it may be destroyed (#1844)
                            chart.hoverPoint.firePointEvent('click', e);
                        }
                        this.pinch(e); //21456: ChartCanvas: Upgrade Highcharts to 3.0.10
                    }
	...
	onContainerTouchMove: function (e) {
            //LOGIFIX - for mask selection 21456: ChartCanvas: Upgrade Highcharts to 3.0.10
...
Legend.prototype = {
		...
		if (alignAttr) {
			...
			each(this.allItems, function (item) {
				...
				if (checkbox) {
					...
					css(checkbox, {
						left: (alignAttr.translateX + item.checkboxOffset + checkbox.x - 20) + PX, //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework and reverting of formatters changes
						top: top + PX,
		...
		each(this.chart.series, function (series) {
                ...
                if (series.options && series.options.data) {
                    for (var i = 0; i < series.options.data.length; i++) {
                        ...
                        if (dataObject && dataObject.qt && dataObject.qt[1]) {
			...
			if (!series.chart.userOptions.isChartCanvas || scrap.some(function (item) {
			    ...
			    if (!series.color && !series.legendColor) {
			        scrap.some(function (item) {
			            series.legendColor = item.color;
			        })
			    }
	...
	render: function () {
		var legend = this,
            ...
            //LOGIFIX -> code was moved here 22191 ChartCanvas: Reverting of an upgrade to v4 framework
		...
		if (legendHeight > spaceHeight && !options.useHTML) {
			...
			each(allItems, function (item, i) {
				var y = item._legendItemPos[1],
					...
					h =item.legendItem.getBBox() ? mathRound(item.legendItem.getBBox().height) : 0,
					len = pages.length;
...
Chart.prototype = {
	...
	init: function (userOptions, callback) {
		...
		chart.animation = useCanVG && !this.userOptions.isExportRender ? false : pick(optionsChart.animation, true);
		chart.pointCount = chart.colorCounter = chart.symbolCounter = 0;

		chart.firstRender();

	    //LOGIFIX - SVG title attribute
		var titleAsDesc = true;
		if (chart.renderer && chart.userOptions.title && chart.userOptions.title.text) {
		    desc = chart.renderer.createElement('title').add();
		    var titleText = document.createTextNode(chart.userOptions.title.text);
		    desc.element.innerHTML = chart.userOptions.title.text;
		    titleAsDesc = false;
		}

	    //LOGIFIX - SVG description attribute
		if (chart.renderer && chart.userOptions.chart.altText) {
		    desc = chart.renderer.createElement('desc').add();
		    var descText = document.createTextNode(chart.userOptions.chart.altText);
		    desc.element.appendChild(descText);

		    if (titleAsDesc) {
		        desc = chart.renderer.createElement('title').add();
		        var titleText = document.createTextNode(chart.userOptions.chart.altText);
		        desc.element.appendChild(titleText);
		    }
		}
	...
	redraw: function (animation) {
		...
		if (legend && redrawLegend && legend.options.enabled) { // series or pie points are added or removed //LOGIFIX SVG gauges tick fixes and Arc gauge font size fixes.
	    ...
	    var axis = axes[0], //LOGIFIX SVG gauges tick fixes and Arc gauge font size fixes.
        ticks = axis.ticks,
        horiz = axis.horiz,
        options = axis.options,
        overflow = options.labels.overflow,
		...
		if (justifyLabels) {
		    if (!axis.options.holdLastTick) {//LOGIFIX - labels and ticks in SVG gauges
		        ...
		        if (/*sortedPositions[sortedPositions.length - 1] > axis.options.max*//*ticks.length > 0*/isDirtyBox) {
		            if (tickPositions.length > 0) {
		                ticks[sortedPositions[sortedPositions.length - 1]].pos = axis.options.max;
		                var newLastTick = ticks[sortedPositions[sortedPositions.length - 1]];
		                ticks[sortedPositions[sortedPositions.length - 1]].destroy();
	...
	layOutTitles: function (redraw) {
		...
		if (title) {
			title
				...
				.align(extend({ 
				    y: (titleOptions.verticalAlign == 'bottom') ? 0 : //LOGIFIX position of text when valgin = bottom
                        renderer.fontMetrics(titleOptions.style.fontSize, title).b - 3
				}, titleOptions), false, 'spacingBox');
			
			if (!titleOptions.floating && !titleOptions.verticalAlign) {
				titleOffset = title.getBBox().height;
			} else if (!titleOptions.floating) { //LOGIFIX - offset for title when valign = bottom
			    if(titleOptions.verticalAlign == 'top') {
			        titleOffset = title.getBBox().height;
			    } else if (titleOptions.verticalAlign == 'bottom') {
			        this.extraBottomMargin = title.getBBox().height;
			    }
			}
		}
		if (subtitle) {
		    ...
		    subtitle
				.css({ width: (subtitleOptions.width || autoWidth) + PX })
                    .align(extend({ y: subtitleOffset }, subtitleOptions), false, 'spacingBox'); //LOGIFIX Upgrading HighCharts framework to 4.0.4
	...
	getMargins: function () {
		...
		// Adjust for legend
		if (legend && legend.display && !legendOptions.floating) { //LOGIFIX SVG gauges tick fixes and Arc gauge font size fixes.
	...
	setSize: function (width, height, animation) {
        ...
        //LOGIFIX - custom event for scaling of axes
		fireEvent(chart, 'setSize');
...
Series.prototype = {
			...
			while (i--) {
				...
				if (hasPointSpecificOptions) {
					...
					if (!seriesOptions.marker) { // column, bar, point
					    ...
					    pointStateOptionsHover.color = pointStateOptionsHover.color || (point.options && !point.options.color && stateOptionsHover.color) || //LOGIFIX 22191: Restoring 22385, 22495,21302
							Color(point.color)
	...
	getGraphPathPrototypeAreaSpline: function (points, nullsAsZeroes, connectCliffs) {
	    var series = this,
            options = series.options,
            step = options.step,
            reversed,
	...
	getSegmentPathAreaSpline: function (points) {
	    var getGraphPath = this.getGraphPathPrototypeAreaSpline,
            graphPath,
            options = this.options,
            stacking = options.stacking,
            yAxis = this.yAxis,
            ...
            addDummyPoints = function (i, otherI, side) {
                ...
                if (stacks && stacks[point.x] && stacks[point.x].points) {
                    for (x in stacks[point.x].points) {
                        if (x.split(',')[0] == seriesIndex) {
                            stackedValues = stacking && stacks[point.x].points[x];
                            break;
                        }
                    }
                }
	    ...
	    if (stacking) {
	        points = this.getStackPointsAreaSpline();
	    }
	    ...
	    for (i = 0; i < points.length; i++) {
	        isNull = points[i].isNull = points[i].x === null || !isNumber(points[i].y, true);
	...
	getStackPointsAreaSpline: function () {
	    var series = this,
            segment = [],
            keys = [],
            xAxis = this.xAxis,
	...
	render: function () {
		...
		if (options.clip !== false && !series.sharedClipKey && !hasRendered) {
		    var ie = isIEFunction(); //LOGIFIX 22307 Analysis Grid:`Member not found' js error is shown on resizing Heatmap  in IE7
		    if (ie && ie == 7 || ie == 8) {
		        var maxValue = window.screen.availWidth > window.screen.availHeight ? window.screen.availWidth : window.screen.availHeight;
		        chart.clipRect.height = maxValue;
		        chart.clipRect.width = maxValue;
...
var areaProto = AreaSeries.prototype,
	AreaSplineSeries = extendClass(SplineSeries, {
		...
		getSegmentPath: areaProto.getSegmentPathAreaSpline,
		closeSegment: areaProto.closeSegment,
		drawGraph: areaProto.drawGraphAreaSpline,
		drawLegendSymbol: LegendSymbolMixin.drawRectangle
	});
...
seriesTypes.pie = PieSeries;
    Series.prototype.trimDataLabels = function (str) { //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework
        var series = this;
        return str;
    };
...
Series.prototype.alignDataLabel = function (point, dataLabel, options, alignTo, isNew) {
	...
	if (visible) {
		...
		if (options.rotation) { // Fancy box alignment isn't supported for rotated text
		    ...
		    dataLabel.align(options, null, alignTo, point); //LOGIFIX Issue 21976 and Revert "Issue 21976 Data labels (Alignment horizontal and vertical) are not working for chart series"
...
if (seriesTypes.pie) {
	seriesTypes.pie.prototype.drawDataLabels = function () {
		...
		each(data, function (point) {
            ...
            //LOGIFIX -> all data labels are empty, nothing to calculate 20890: ChartCanvas: crosstabbed stacked AreaSeries are in wrong order.
            i = 0;
	...
	drawTrackerPoint: function () {
		var series = this,
			...
			onMouseOver = function (e) {
				...
				if (point !== UNDEFINED && point !== chart.hoverPoint && point.onMouseOver) { // undefined on graph in scatterchart //LOGIFIX
		...
		if (!series._hasTracking) {
			each(series.trackerGroups, function (key) {
			    if (series[key] && pointer) { // we don't always have dataLabelsGroup //LOGIFIX SVG gauges tick fixes and Arc gauge font size fixes.
					series[key]
	...
	(useHTML ? legendItem : item.legendGroup).on('mouseover', function () {
		...
		.on('mouseout', function () {
			legendItem.css(item.visible ? itemStyle : itemHiddenStyle);
                    //LOGIFIX #21195 Fixed bug: after mouseover legend still display bold
                    itemStyle["fontWeight"] = weight;
	...
	zoom: function (event) {
		...
		if (displayButton) {
		    ...
		    if (btn) {
		        ...
		        btn.setAttribute("collapsed-button", "True");
		    }		    
		}
...
extend(Point.prototype, {
	...
	select: function (selected, accumulate) {
		...
		point.firePointEvent(selected ? 'select' : 'unselect', { accumulate: accumulate }, function () {
			...
			point.setState(selected && SELECT_STATE);
                ...
                //LOGIFIX we don't need this 22191 LOGIFIX descriptions added to code changes (fixes) introduced by LOGI developers
                // mask selection 
			// unselect all other points unless Ctrl or Cmd + click
			//LOGIFIX 21456: ChartCanvas: Upgrade Highcharts to 3.0.10
                //if (!accumulate) {
            ...
            //LOGIFIX 21456: ChartCanvas: Upgrade Highcharts to 3.0.10
            fireEvent(this.series, 'pointselection', this);
	},
	...
	setState: function (state, move) {
		...
		if (haloOptions && haloOptions.size && point.haloPath) { //LOGIFIX Issue ChartCanvas: heatmap -> mouse hover -> js exception
			if (!halo) {
	...
	haloPath: function (size) {
		...
		return chart.renderer.symbols.circle(
			plotBox.translateX + (inverted ? (series.yAxis ? series.yAxis.len - this.plotY : this.plotY) : this.plotX) - size, 
			plotBox.translateY + (inverted ? (series.xAxis ? series.xAxis.len - this.plotX : this.plotX) : this.plotY) - size, 
			size * 2, 
			size * 2
		);
	}
