YUI.add('droppableContainer', function (Y) {
    ...
    Y.namespace('LogiXML').DroppableContainer = Y.Base.create('DroppableContainer', Y.Base, [], {
        ...
        onDropHit: function (e) {
            var prevSibling,
                node = null, obj, act, editorId, guid = this.getGuid(),
                obj = e.drag.get('node').getData("rdDraggableElement"), dropStub;
            if (!obj) {
                return;
            }
            prevSibling = this.removeGhost();
            act = obj.action;
            ...
            if (!obj.preventCloneNode) {
                if (obj.proxyHideOnStart != true) {
                    if (obj.dropStubId && obj.dropStubId != "") {
                        dropStub = Y.one('#' + obj.dropStubId);
                        node = dropStub.cloneNode(true);
                        node.show();
                        node.setStyle('height', e.drag.get('node').getAttribute('offsetHeight'));
                    } else {
                        node = e.drag.get('node').cloneNode(true);
                    }
                    node.setAttribute("id", editorId);
                } else {
                    node = e.drag.get('node');
                }
                if (prevSibling) {
            ...
            if (node && obj.proxyHideOnStart) {
                ...
                for (; i < length; i++) {
                    ...
                    chartNode.hide();
                    parentWidth = chartNode.ancestor('.rdDroppableContainer').get('offsetWidth');
                    chartNode.show();
